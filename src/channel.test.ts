...
function requestChannelremoveowner(token: string, channelId: number, uId: number) {
  return requestHelper('POST', '/channel/removeowner', {token, channelId, uId});
}
...
describe ('Testing channelLeaveV1', () => {
    ...
    beforeEach(() => {
      ...
      test ('allowing a member of a channel to leave the channel', () => {
        const user1 = requestAuthRegister("wrongemail@gmail.com", "badpassword", "wrong", "email");
        const token1 = tokenCreate(user1.email);
        const channel1 = requestChannelscreate(token1, 'test',true).channelId;
        const result = requestChannelleavev1(token1, channel1);
      ...
      test ('token generated is invalid', () => {
        ...
        const result = channelLeaveV1('wrong token', channel1);
      ...
      test ('channelId does not refer to a valid channel', () => {
        const user1 = requestAuthRegister("wrongemail@gmail.com", "badpassword", "wrong", "email");
        const result = channelLeaveV1(user1.token, -1);
      ...
      test ('user is not a member of the channel', () => {
        const user1 = requestAuthRegister("wrongemail@gmail.com", "badpassword", "wrong", "email");
        const channel1 = requestChannelscreate(user1.token, 'test',true).channelId;
        const result = channelLeaveV1(user1.token, channel1);
...
describe ("Testing the addowner function", () => {
    ...
    beforeEach(() => {
      ...
      test ('Adding a user as an owner to the channel', () => {
        const user1 = requestAuthRegister("wrongemail@gmail.com", "badpassword", "wrong", "email");
        const channel1 = requestChannelscreate(user1.token, 'test',true).channelId;
        const result = requestChanneladdowner(user1.token, channel1, user1)
        expect(result).toEqual({});
      ...
      test('should return error with invalid token', () => {
        ...
        const result = requestChanneladdowner(token, channel1, user1);
        expect(result).toEqual({ error: 'token is invalid' });
      });
      ...
      test('should return error with invalid channelId', () => {
        ...
        const result = requestChanneladdowner(user1.token, channelId, user1);
        expect(result).toEqual({ error: 'channelId is not part of a valid channel' });
      });
      ...
      test('should return error with invalid uId', () => {
        ...
        const channel1 = requestChannelscreate(user1.token, 'test',true).channelId;
        const userId = -999;
        const result = requestChanneladdowner(user1.token, channel1, userId);
      ...
      test('should return error with uId already an owner', () => {
        const user2 = requestAuthRegister("desmondloh@gmail.com", "bad", "desmond", "loh");
        const channel2 = requestChannelscreate(user2.token, 'test1', false).channelId; 
 
        const result = requestChanneladdowner(user2.token, channel2, user2);
      ...
      test('should return error with user without owner permissions', () => {
        const user2 = requestAuthRegister("desmondloh@gmail.com", "bad", "desmond", "loh");
        const channel1 = requestChannelscreate(user2.token, 'test',true).channelId;
       
        const result = requestChanneladdowner(user2.token, channel1, user2);
...
describe ("Testing the removeowner function", () => {
    beforeEach(() => {
      ...
      test ('removing an owner from the channel', () => {
        ...
        const result = requestChannelremoveowner(user1.token, channel1, user1);
        expect(result).toEqual({});
      ...
      test('should return error with invalid token', () => {
        ...
        const token = 'not a string'
        const result = requestChannelremoveowner(token, channel1, user1);
        expect(result).toEqual({ error: 'token is invalid' });
      });
      ...
      test('should return error with invalid channelId', () => {
        ...
        const channelId = 999;
        const result = requestChannelremoveowner(user1.token, channelId, user1);
        expect(result).toEqual({ error: 'channelId is not part of a valid channel' });
      });
      ...
      test('should return error with invalid uId', () => {
        ...
        const userId = 999;
        const result = requestChannelremoveowner(user1.token, channel1, userId);
        expect(result).toEqual({ error: 'user is not valid' });
      });
      ...
      test('should return error with uId not in channel', () => {
        ...
        const result = requestChannelremoveowner(user2.token, channel2, user2);
        expect(result).toEqual({ error: 'user is not a member of the channel' });
      });
      ...
      test('should return error with uId where user is not an owner of the channel', () => {
        ...
        const result = requestChannelremoveowner(user2.token, channel1, user2);
