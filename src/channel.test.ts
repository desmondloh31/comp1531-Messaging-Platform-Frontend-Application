...
describe ("Testing channelDetails Final Test batch", () => {
    test ('testing if authUserId is not valid', () => {
        const result = channelDetailsV1(4,5);
    ...
    test ('testing if authUserId and channelId are valid', () => {
        ...
        const channel1 = channelsCreateV1(5,'channel1', false);
        const channel2 = channelsCreateV1(5,'channel2', false);
        const result = channelDetailsV1(5,data.channels[1].channelId);
    ...
    test ('testing if channelId is not valid', () => {
        ...
        const userID = authRegisterV1("example@gmail.com", "abc123", "John", "Smith");
        const channel1 = channelsCreateV1(5,'channel1', false);
        const result = channelDetailsV1(5,5);
    ...
    test ('testing if channelId is valid but authuser is not a part of the channel', () => {
        ...
        const channel1 = channelsCreateV1(5,'channel1', false);
        const result = channelDetailsV1(5, data.channels[0].channelId);
        expect(result).toEqual({error: 'User is not a part of the channel or invalid channelId'});
    });
});
...
describe ("Testing channelJoinV1", () => {
    beforeEach (() => {
        ...
        channelsCreateV1(expect.any(Number), "channel1", true);
        channelsCreateV1(expect.any(Number), "channel2", false);
    });
    ...
    test ('channelId is invalid', () => {
        ...
        let channelId2 = -1;
        const result = channelJoinV1(expect.any(Number), channelId2);
        expect(result).toStrictEqual(ERROR);
    ...
    test ('Authorised user successfully joins channel', () => {
      ...
      let channelId1 = 0;
      const result = channelJoinV1(expect.any(Number), channelId1);
      expect(result).toStrictEqual({});
    }) 
})
